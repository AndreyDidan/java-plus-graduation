...
import ru.practicum.kafka.SimilarityEventProducer;
...
public class AggregationStarter {
    ...
    private void handleRecord(ConsumerRecord<String, UserActionAvro> record) throws InterruptedException {
        ...
        if (!result.isEmpty()) {
            log.info("СТАРТ публикации, результатов: {}", result.size());

            Producer<String, SpecificRecordBase> producer = similarityEventProducer.getProducer();

            for (EventSimilarityAvro event : result) {
                String key = event.getEventA() + ":" + event.getEventB();
                ProducerRecord<String, SpecificRecordBase> producerRecord =
                        new ProducerRecord<>(appConfig.getTopics().getSimilarityTopic(), key, event);

                producer.send(producerRecord, (metadata, ex) -> {
                    if (ex != null) {
                        log.error("Ошибка при отправке события [{}]: {}", key, ex.getMessage(), ex);
                    } else {
                        log.info("Отправлено событие [{}] в partition={}, offset={}", key, metadata.partition(), metadata.offset());
                    }
                });
            }

            producer.flush();
            log.info("Флаш завершён, все события отправлены");
        } else {
            log.info("result is empty — ничего не отправляется");
        }
    }
}
*/